...
namespace chipotto
{
	...
	public:
		...
		uint16_t GetStackAddress() const;
		uint8_t GetRegisters(uint8_t rs);
		uint16_t GetI() const;
		uint8_t GetMemoryMapping(uint16_t index);
        bool GetSuspended() const;
		uint8_t GetWaitForKeyboard() const;
		uint8_t GetDelayTimer() const;
		uint64_t GetDeltaTick() const;
